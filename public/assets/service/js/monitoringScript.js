const modalFunction = () => {
    ...
    bak.forEach(item => {
        item.addEventListener('click', (e) => {
            const data = item.dataset.bak;
            console.log(data)

            if(data !== '-'){
                modal2.showModal();
                modalImage.src = `${wrapper.dataset.uri}/images/bak/${data}`;
                modalImage2.src = '';
            }
        });
    });
...
const displayFunction = (data, wrapper, val) => {
    ...
    for(const item of data){
        html += `<div class="grid grid-cols-10 text-sm mt-5 gap-2">
                    ...
                    <div>
                        ${val === 'finished' ? (
                            `<a class="cursor-pointer delete" data-href="/superuser/monitoring/delete/status/${item.id}"  data-name="${item.name}"><i class="fa-solid fa-trash"></i></a>`
                        ) : (
                            `<a class="me-4 cursor-pointer update" data-href="/superuser/monitoring/update/status/${item.status}/${item.id}/${item.plan_id}" data-name="${item.name}"><i class="fa-solid fa-pen-to-square"></i></a>
                            <a class="cursor-pointer delete" data-href="/superuser/monitoring/delete/status/${item.id}" data-name="${item.name}"><i class="fa-solid fa-trash"></i></a>`
                        )}
                    </div>
                </div>`;
...
const modal1 = document.querySelectorAll('dialog');

uploadBtn.forEach(item => {
    item.addEventListener('click', function(){
        const data = item.dataset.data;

        modal1[modal1.length - 1].showModal(); 
        document.querySelector('.form').action = `/superuser/upload/bak/${data}`;
        console.log(document.querySelector('.form'));
    });
});

closeUploadBtn.addEventListener('click', function(){
    modal1[modal1.length - 1].close(); 
});