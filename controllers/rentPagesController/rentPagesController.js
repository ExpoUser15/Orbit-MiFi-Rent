...
const rentController = async (req, res) => {
    try {
        ...
        if (success === true) {
            ...
            success = false;
            return res.render('index.ejs', { data: { path, notif, success: true }, jumlah: ['0', '0'], title: 'Telkomsel Orbit MiFi Rent', uri: `${req.protocol}://${req.hostname}:${port}` });
        }else if(success === 'not available'){
            const { notif } = notification('Modem is not available!', 'alert_10265049.png', 'from-yellow-300 to-orange-400');
            success = false;
            return res.render('index.ejs', { data: { path, notif, success: true }, jumlah: ['0', '0'], title: 'Telkomsel Orbit MiFi Rent', uri: `${req.protocol}://${req.hostname}:${port}` });
        }
...
const rentPostController = async (req, res) => {
    try {
        ...
        if (!body.name || !body.destination || !files.passport || !body.modem || !body.plan || !body.modemPrice || !body.price || !body.total || !body.phone) {
            success = false;

            return res.redirect('/rent');
        }

        const location = await sequelize.query('SELECT tb_locations.location, tb_ready_stoks.n1, tb_ready_stoks.n2 FROM tb_ready_stoks JOIN tb_locations ON tb_locations.location_id = tb_ready_stoks.location WHERE tb_locations.location = :location', {
            type: QueryTypes.SELECT,
            replacements: { location: body.destination }
        });
        
        if(body.modem === 'N1' && location[0].n1 === '0'){
            success = 'not available';

            return res.redirect('/rent');
        }

        if(body.modem === 'N2' && location[0].n2 === '0'){
            success = 'not available';
            ...
            if (planIdTest) {
                ...
                sequelize
                    ...
                    .then(async () => {
                        ...
                        if(body.modem === 'N1'){
                            const substract = String(Number(location[0].n1) - 1);
                            console.log(substract);
                            await sequelize.query('UPDATE tb_ready_stoks rs JOIN tb_locations l ON l.location_id = rs.location SET n1 = :n1 WHERE l.location = :location', {
                                type: QueryTypes.UPDATE,
                                replacements: { n1: substract, location: location[0].location }
                            });
                        }

                        if(body.modem === 'N2'){
                            const substract = String(Number(location[0].n2) - 1);
                            console.log(substract);
                            await sequelize.query('UPDATE tb_ready_stoks rs JOIN tb_locations l ON l.location_id = rs.location SET n2 = :n2 WHERE l.location = :location', {
                                type: QueryTypes.UPDATE,
                                replacements: { n2: substract, location: location[0].location }
                            });
                        }
                    })
                    .catch((error) => {
